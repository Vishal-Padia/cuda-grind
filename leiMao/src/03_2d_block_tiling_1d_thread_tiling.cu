...
__global__ void gemm_v03(size_t m, size_t n, size_t k, T alpha, T const* A, size_t lda, T const* B, size_t ldb, T beta, T* C, size_t ldc)
{
    // using a compile time constant for the number of threads per block
    ...
    // each thread in the block computes the BLOCK_TILE_SIZE_Y output values
...
#pragma unroll
            for (size_t k_i{0U}; k_i < BLOCK_TILE_SIZE_K; ++k_i)
            {
                ...
                // b_val is cached in the regsiter to alleviate the pressure on the shared memory access
                T const b_val{B_thread_block_tile[B_thread_block_tile_row_idx][threadIdx.x % BLOCK_TILE_SIZE_X]};
#pragma unroll
