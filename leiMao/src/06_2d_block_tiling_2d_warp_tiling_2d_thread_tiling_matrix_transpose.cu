...
__global__ void gemm_v06_vectorized(size_t m, size_t n, size_t k, T alpha, T const* A, size_t lda, T const* B, size_t ldb, T beta, T* C, size_t ldc)
{
    // static assertions
    static_assert(NUM_THREADS_PER_WARP_X * NUM_THREADS_PER_WARP_Y == 32U); // warp is always 32 threads; this verifies that grid layout matches the hardware 
    
    // block tile must tile evenly into warps
    ...
    // thread tile must fit evenly in a warp tile along both x and y dimensions
    ...
    // warp tile must be perfectly divisible by (thread tile * thread count) meaning there shouldn't be any threadload imbalance
    ...
    __shared__ T A_thread_block_tile_transposed[BLOCK_TILE_SIZE_K][BLOCK_TILE_SIZE_Y]; // transposed from  natural row-major
    __shared__ T B_thread_block_tile[BLOCK_TILE_SIZE_K][BLOCK_TILE_SIZE_X];

    // A_vals is cached in the register -- each thread keeps it's slice of A and B in registers
    ...
    size_t const thread_linear_idx_in_warp{thread_linear_idx % 32U}; // linear index of the thread in the warp
    size_t const thread_linear_row_idx_in_warp{thread_linear_idx_in_warp / NUM_THREADS_PER_WARP_X}; // row index of the thread in the warp
    size_t const thread_linear_col_idx_in_warp{thread_linear_idx_in_warp % NUM_THREADS_PER_WARP_X}; // column index of the thread in the warp

    // number of thread block tiles needed to cover the entire k dimension
    size_t const num_thread_block_tiles{(k + BLOCK_TILE_SIZE_K - 1) / BLOCK_TILE_SIZE_K};
    
    // results accumulated in registers for each thread
    // thread_tile_size_y = 8, thread_tile_size_x = 8, num_thread_tiles_per_warp_y = 8, num_thread_tiles_per_warp_x = 4
    // 8x8x8x4 = 2048 float32s = 8kb per thread
    ...
    // vectorized memory access --> if T = float32: int4 = 16 bytes, float32 = 4 bytes -> num_vector_units = 4
    constexpr size_t NUM_VECTOR_UNITS{sizeof(int4) / sizeof(T)};
    static_assert(sizeof(int4) % sizeof(T) == 0U);
    
    // K and X dimensions must be divisible by vectorization width; no partial vector loads
    static_assert(BLOCK_TILE_SIZE_K % NUM_VECTOR_UNITS == 0U);
    static_assert(BLOCK_TILE_SIZE_X % NUM_VECTOR_UNITS == 0U);

    // Instead of loading 8 floats individually, load them as 2 int4 chunks
    ...
    // Tiling over K, since we can't fit all of K into shared memory at once, we iterate over K in chunks of BLOCK_TILE_SIZE_K
    // what happens in each iteration?
    // 1. load a block_tile_size_k - wide slice of A and B into shared memory
    // 2. All threads in a block compute their piece using that slice
    // 3. Accumulate results into C_thread_results
    // 4. Move to the next K slice, repeat
...
#pragma unroll
            ...
            // grab one A value (A_vals[row][y]) and one B value (B_vals[col][x])
            // multiply and accumulate into C

            // all unrolled, so at compile time we get NUM_THREAD_TILES_PER_WARP_Y × NUM_THREAD_TILES_PER_WARP_X × THREAD_TILE_SIZE_Y × THREAD_TILE_SIZE_X multiplications, unrolled into straight-line code
            for (size_t thread_tile_repeat_row_idx{0U}; thread_tile_repeat_row_idx < NUM_THREAD_TILES_PER_WARP_Y; ++thread_tile_repeat_row_idx)
            {
#pragma unroll
                for (size_t thread_tile_repeat_col_idx{0U}; thread_tile_repeat_col_idx < NUM_THREAD_TILES_PER_WARP_X; ++thread_tile_repeat_col_idx)
...
#pragma unroll
    // same nesting as the compute loop, but we iterate over vectorized chunk in X (not individual elements)
    for (size_t thread_tile_repeat_row_idx{0U}; thread_tile_repeat_row_idx < NUM_THREAD_TILES_PER_WARP_Y; ++thread_tile_repeat_row_idx)
    {
#pragma unroll
        for (size_t thread_tile_repeat_col_idx{0U}; thread_tile_repeat_col_idx < NUM_THREAD_TILES_PER_WARP_X; ++thread_tile_repeat_col_idx)
...
#pragma unroll
                for (size_t thread_tile_x_vector_idx{0U}; thread_tile_x_vector_idx < VECTORIZED_THREAD_TILE_SIZE_X; ++thread_tile_x_vector_idx)
                {
                    // Map from local thread coords back to globla matrix coords
                    ...
                    size_t const C_col_idx{blockIdx.x * BLOCK_TILE_SIZE_X + warp_col_idx * WARP_TILE_SIZE_X + thread_tile_repeat_col_idx * (WARP_TILE_SIZE_X / NUM_THREAD_TILES_PER_WARP_X) + thread_linear_col_idx_in_warp * THREAD_TILE_SIZE_X + thread_tile_x_vector_idx * NUM_VECTOR_UNITS};

                    // boundary check so that we don't go out of bounds
                    if (C_row_idx < m && C_col_idx < n)
                    {
                        // load 16 bytes (4 floats) from C as a single int4 chunk --> basically vectorized read from DRAM
                        int4 C_vals{*reinterpret_cast<int4 const*>(&C[C_row_idx * ldc + C_col_idx])};
#pragma unroll
                        // For each chunk of the 4 values in the int4 chunk:
                        // 1. Scale computed result by alpha
                        // 2. Add beta x old C value
                        // 3. Store back in the int4 chunk
                        for (size_t i{0U}; i < NUM_VECTOR_UNITS; ++i)
                        {
                            reinterpret_cast<T*>(&C_vals)[i] = alpha * C_thread_results[thread_tile_repeat_row_idx][thread_tile_repeat_col_idx][thread_tile_y_idx][thread_tile_x_vector_idx * NUM_VECTOR_UNITS + i] + beta * reinterpret_cast<T const*>(&C_vals)[i];
                        }
