    ...
    )
{
    ...
    // skew size could affect the data alignment in shared memory when we use vectorized load
...
#pragma unroll
    for (size_t load_idx{0U}; load_idx < (BLOCK_TILE_SIZE_Y * VECTORIZED_BLOCK_TILE_SIZE_K + NUM_THREADS - 1U)/ NUM_THREADS;
        ++load_idx)
    {
...
// the dreaded kernel itself
template <typename T, size_t BLOCK_TILE_SIZE_X, size_t BLOCK_TILE_SIZE_Y,
          size_t BLOCK_TILE_SIZE_K, size_t THREAD_TILE_SIZE_X,
          size_t THREAD_TILE_SIZE_Y>
__global__ void gemm_v05_vectorized(size_t m, size_t n, size_t k, T alpha, T const* A, size_t lda, T const* B, size_t ldb, T beta, T* C, size_t ldc)
{
    // number of threads per block
    constexpr size_t NUM_THREADS{BLOCK_TILE_SIZE_X * BLOCK_TILE_SIZE_Y / (THREAD_TILE_SIZE_X * THREAD_TILE_SIZE_Y)};
    size_t const thread_linear_idx{threadIdx.y * blockDim.x + threadIdx.x};

    // cache tile of a and b in the shared memory for data resuse
    ...
    // a vals are cache in the register
    ...
    {
        load_data_to_shared_memory_transposed_vectorized<T, BLOCK_TILE_SIZE_X, BLOCK_TILE_SIZE_Y, BLOCK_TILE_SIZE_K, NUM_THREADS>(A, lda, B, ldb, A_thread_block_tile_transposed, B_thread_block_tile, thread_block_tile_idx, thread_linear_idx, m, n, k);
        __syncthreads();
...
#pragma unroll
    ...
    // vectorized writing the results to the DRAM
    for (size_t thread_tile_row_idx{0U}; thread_tile_row_idx < THREAD_TILE_SIZE_Y; ++thread_tile_row_idx)
    {
        for (size_t thread_tile_col_vector_idx{0U}; thread_tile_col_vector_idx < VECTORIZED_THREAD_TILE_SIZE_X; ++thread_tile_col_vector_idx)
        {
            ...
            // vectorized read from X
            int4 C_row_vector_vals{*reinterpret_cast<int4 const*>(&C[C_row_idx * ldc + C_col_idx])};
            // vectorized read from C_thread_results
            int4 const C_thread_results_row_vectors_vals{*reinterpret_cast<int4 const*>(&C_thread_results[thread_tile_row_idx][thread_tile_col_vector_idx * NUM_VECTOR_UNITS])};

            // update the values in teh C_row_vector_vals
            for (size_t i{0U}; i < NUM_VECTOR_UNITS; ++i)
            {
                reinterpret_cast<T*>(&C_row_vector_vals)[i] = alpha * reinterpret_cast<T const*>(&C_thread_results_row_vectors_vals)[i] + beta * reinterpret_cast<T const*>(&C_row_vector_vals)[i];
            }

            // vectorized write to C
            if (C_row_idx < m && C_col_idx < n)
            {
                // no need to mask the out of bound invalid elems
                // beacuse the row of C matrix is 32 byte aligned
