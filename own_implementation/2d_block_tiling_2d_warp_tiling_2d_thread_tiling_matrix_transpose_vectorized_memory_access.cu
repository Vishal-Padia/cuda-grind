...
__global__ void two_dim_tiling_two_dim_warp_tiling_two_dim_thread_matrix_transpose_vec_memory_access_kernel(size_t m, size_t n, size_t k, T alpha, T const* A, size_t lda, T const* B, size_t ldb, T beta, T* C, size_t ldc)
{
    ...
    // all threads compute their part using that slice
    // accumulate results
    // move to the next K slice
    for(size_t thread_block_tile_idx{0U}; thread_block_tile_idx < num_thread_block_tiles; ++thread_block_tile_idx)
    {
        ...
        for(size_t k_i{0U}; k_i < BLOCK_TILE_SIZE_K; ++k_i)
        {
            ...
            {
                size_t const A_thread_block_tile_row_idx{warp_row_idx * WARP_TILE_SIZE_Y + thread_tile_repeat_row_idx * (WARP_TILE_SIZE_Y / NUM_THREADS_PER_WARP_Y) + thread_linear_row_idx_in_warp * THREAD_TILE_SIZE_Y};
...
template <typename T>
void launch_two_dim_tiling_two_dim_warp_tiling_two_dim_thread_matrix_transpose_vec_memory_access_kernel(size_t m, size_t n, size_t k,
    ...
    T* C, size_t ldc, cudaStream_t stream)
    {
        ...
        dim3 const grid_dim{
            ...
            (static_cast<unsigned int>(m) + BLOCK_TILE_SIZE_Y - 1U) /
                BLOCK_TILE_SIZE_Y,
            1U};
        two_dim_tiling_two_dim_warp_tiling_two_dim_thread_matrix_transpose_vec_memory_access_kernel<T, BLOCK_TILE_SIZE_X, BLOCK_TILE_SIZE_Y,
    ...
    template void launch_two_dim_tiling_two_dim_warp_tiling_two_dim_thread_matrix_transpose_vec_memory_access_kernel<float>(
    ...
    template void launch_two_dim_tiling_two_dim_warp_tiling_two_dim_thread_matrix_transpose_vec_memory_access_kernel<double>(
        ...
        size_t ldc, cudaStream_t stream);
    template void launch_two_dim_tiling_two_dim_warp_tiling_two_dim_thread_matrix_transpose_vec_memory_access_kernel<__half>(
